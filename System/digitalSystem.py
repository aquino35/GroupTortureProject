...
class DigitalSystem():
    ...
    def __init__(self, layered_comp_list, num_of_runs):
        ...
        num_of_runs (int): number the user wants to run the system
        """
    ...
    def organize(self):
        ...
        :return
        list: Containing the order of dependencies of the whole network.
        """
    ...
    def traverse(self, dig_comp):
        """This is a helper function used in the Organize function that goes recursively around the structure
        evaluating the dependencies of a single digital component.
        :parameter
        dig_comp (component): a component inside the network to be evaluated.
        """
        self.visited[dig_comp] = True  # base case
        for dig_comp_in in self.network_dict[dig_comp]:  # We traverse each node inside the connection dictionary that is given
            if not self.visited[dig_comp]:
